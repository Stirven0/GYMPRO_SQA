...
| D-003 | 20/06/2026 | API | Media | **Input malformed JSON causa HTTP 500**: Enviar `{"email":{"$ne":""},"password":{"$ne":""}}` al login produce error interno 500 (el backend es SQLite, no NoSQL; el error es por falta de validación del payload JSON). | Abierto |
